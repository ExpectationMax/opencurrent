...
bool Eqn_IncompressibleNS3D<T>::advance_one_step(double dt)
{
  ...
  //_deriv_tempdt.reduce_sqrsum(tmp);
  //printf("**  3: %.16f\n", tmp);
...
bool Eqn_IncompressibleNS3DCo<T>::advance_one_step(double dt)
{
  ...
  T tmp;
  //_deriv_vdt.co_reduce_sqrsum(tmp);
  ...
  //_deriv_tempdt.co_reduce_sqrsum(tmp);
  //if (ThreadManager::this_image() == 0)
  //  printf("** 3: %.16f\n", tmp);
